...
#interrupt function -the purpose of this function is to set the sleepcount to maximun and display
#a message to the user when this function is called
function Interruptfun {
  sleepCount=$numberOfSleeps
  echo "Hello you interrupted the count you are not allowed to do that"
}

#quit function- the purpose of this function is to display a message to the user and then exit immediately
#when this function is called
function Quitfun {
  echo "congrats, Found the secret to getting out and we are exiting"
  exit 0
}
# Normally traps catch signals and do something useful or necessary with them
# the purpose of this command is to trap the interrupt signal and then call the Interruptfun when the INT signal is trapped.
#Also the purpose of the second command is to tarp the quit signal and then call the Quitfun when a quit signal is traped .
trap Interruptfun SIGINT
trap Quitfun SIGQUIT
