...
#####Task 1####command to test if /etc/resolv.conf is a regular file
test -f /etc/resolv.conf && echo "/etc/resolv.conf exists and is regualr file" || echo "/etc/resolv.conf does not exist "

######TASK 2#### command to check if /etc/resolv.conf file is a symbolic link
test -h /etc/resolv.conf && echo "/etc/resolv.conf exists and is s symbolic link" || echo "/etc/resolv.conf not asymbolic link"

######TASK 3#### command to check if if the /etc/resolv.conf file is a directory
test -d /etc/resolv.conf && echo "/etc/resolv.conf exists and is a directory" || echo "/etc/resolv.conf not a directory"

#####TASK 4#### command to check if the /etc/resolv.conf file is readable
test -r /etc/resolv.conf && echo "/etc/resolv.conf exists and is readable" || echo "/etc/resolv.conf  not readable"

#####TASK 5#### command to check if if the /etc/resolv.conf file is writable
test -w /etc/resolv.conf && echo "/etc/resolv.conf exists and is writeable" || echo "/etc/resolv.conf  not writeable"

#######TASK 6####command to check if the /etc/resolv.conf file is executable
test -x /etc/resolv.conf && echo "/etc/resolv.conf exists and is executable" || echo "/etc/resolv.conf not executable"
...
######TASK 4####command to see if the /tmp directory is readable
[ -r /tmp ] && echo "/tmp is directory is readable" || echo "/tmp is not a readable directory"

######TASK 5####command to see if the /temp directory is writeable
[ -w /tmp ] && echo "/tmp is directory is writable" || echo "/tmp is not a writeable directory"

######TASK 6### command to see if the /tmp directory can be accessed'
[ -x /tmp ] && echo "/tmp is directory can be accesed" || echo "/tmp is not accesed"
...
modfirst=$(($firstNumber % 2))
modsecond=$(($secondNumber % 2))
if [[ $modfirst == 0 ]]; then
  echo "Firsttnumber is even"
else
...
modmulti=$(( $firstNumber % $secondNumber))
if [[ $modmulti == 0 ]]; then
  echo "Firsttnumber is a mutiple of second number"
else
  echo "Firstnumber is not a multiple second number"
fi
...
########TASK 1#####################################
[ -s USER ] && echo "The variable has the name of the user" || echo "The variable is empty"
...
#########TASK 4###########################################
read -p "Enter the first string" astring
read -p "Enter the second string" bstring
[ $astring != $bstring ] && echo "$astring is not equal to $bstring" || echo "$astring is equal to $bstring"
