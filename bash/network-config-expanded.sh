...
############################Task 1 Answer###############################
#defining the array_test
array_test=()
#dynamically generate the network interface name. The loop is used to generate all the network interface names.
for iface in $(ifconfig | cut -d ' ' -f1| tr ':' '\n' | awk NF)
do
  #the if condition checks if the interface name is loopback if its note the the information is collected and stored into an array.
if [[ "$iface" != lo ]]; then
        #command to print the interface name
        printf "$iface\n"
        #command to store the interface name to an array_test
        array_test+=("$iface")
