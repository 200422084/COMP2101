...
##################################Task 1#################################
#command to store the hostname , network interface name , lan address, lan hostname,
#external ip address and external ip address name to corresponding variables.
hostnamevar=$(hostname)
networkinterfacevar=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
lanaddressvar=$(ip a s $networkinterfacevar | awk '/inet /{gsub(/\/.*/,"");print $2}')
lanhostnamevar=$(getent hosts $lanaddressvar| awk '{print $2}')
externalipvar=$(curl -s icanhazip.com)
externalnamevar=$(getent hosts $externalipvar | awk '{print $2}')

################################Task 2#######################################
#command to store the roter ip adress and router hostname to a variable.

routeripvar=$(ip r | awk '/via /{gsub(/\/.*/,"");print $3}')
routernamevar=$(getent hosts $routeripvar | awk '{print $2}')


#command to display the hostname, network interface name, lan address, lan hostname,
#external ip address  ,external name , router ip address and routerhostname
cat <<EOF
Hostname        : $hostnamevar
LAN Address     : $lanaddressvar
LAN Hostname    : $lanhostnamevar
External IP     : $externalipvar
External Name   : $externalnamevar
Router Address  : $routeripvar
Router Hostname : $routernamevar
EOF
